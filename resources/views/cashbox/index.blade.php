                ...
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($payments as $p)
                            <tr><td>{{ $p->created_at }}</td><td>{{ $p->reservation->room->number ?? '-' }}</td><td>{{ \App\Support\Money::format($p->amount, $currency) }}</td><td>{{ ['cash' => 'Espèces', 'airtelmoney' => 'Airtel money', 'mpesa' => 'Mpesa', 'card' => 'Carte', 'credit' => 'À crédit'][$p->payment_method] ?? $p->payment_method }}</td></tr>
                        @empty
